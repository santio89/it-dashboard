...
export default function TDL({ user }) {
  ...
  const editStatusFn = async (task) => {
    ...
    setEditStatus(editStatus => editStatus === "done" ? "not done" : "done")

    const newTask = { ...task, status: editStatus === "done" ? "not done" : "done" }
          ...
          isLoadingTdl ? <div className="loader">Loading...</div> :
            <div className="listWrapper">
              <ul className='tdl-list' ref={listContainer}>
                {
                  tdlList?.map((task) =>
                    <li key={task.id} disabled={taskOptions === task.id && (resultEditTdl.isLoading || isFetchingTdl)}>
                      {/* priority btn */}
                      {taskOptions === task.id &&
                      ...
                      {/* task options */}
                      {
                        ...
                        <div className='tdl-btnContainer' onClick={() => {
                          ...
                          <div className={`tdl-itemData `}>
                            {<>
                              <button title={`Status: ${editStatus ?? "not done"}`} onClick={(e) => { e.stopPropagation(); editStatusFn(task) }}>
                                {editStatus === "done" ? <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check-square" viewBox="0 0 16 16">
                          ...
                          <div className={`tdl-optionsBtns`}>
                            {
                              /* open */
                              < button title={"Info"} onClick={(e) => { e.stopPropagation(); dispatch(setModal({ active: true, data: { modalType: "TDLDataModal", tdlData: true, ...task } })); setListPickerOpen(false) }}>
                        ...
                        <>
                          {
                            taskOptions === task.id ?
                              ...
                              <>
                                <button title={task.title} className={`taskContentBtn ${taskOptions !== task.id && task.priority === "low" && "selectedLow"} ${taskOptions !== task.id && task.priority === "medium" && "selectedMedium"} ${taskOptions !== task.id && task.priority === "high" && "selectedHigh"} ${taskOptions === task.id && "taskOption"} ${task.status === "done" && "taskDone"}`} onClick={() => {
